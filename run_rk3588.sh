...
# Safety and perception feedback must not become silent just because Edge-TTS
# has one transient network failure.  Operators can still explicitly set 0 to
# prefer voice consistency over availability.
export AI_AGENT_TTS_FAILOVER_RETRIES="${AI_AGENT_TTS_FAILOVER_RETRIES:-1}"
export AI_AGENT_TTS_OFFLINE_FALLBACK="${AI_AGENT_TTS_OFFLINE_FALLBACK:-1}"
