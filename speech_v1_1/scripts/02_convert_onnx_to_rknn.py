#!/usr/bin/env python3
"""Convert the fixed-shape SenseVoice ONNX model to non-quantized RKNN."""
...
def main() -> int:
    ...
    if args.onnx.stat().st_size == 0:
        raise RuntimeError(f"ONNX file is empty: {args.onnx}")
    ...
    try:
        print(f"ONNX: {args.onnx} ({args.onnx.stat().st_size} bytes)")
        ...
        require_success(
            "rknn.build", rknn.build(do_quantization=False)
        )
